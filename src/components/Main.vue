    ...
    <v-row>
      ...
      >
        <v-row>
          ...
          >
            ...
            <v-snackbar
              v-model="isShowingQuerySnackbar"
              timeout="4000"
              color="success"
              align="center"
            >
        ...
        <v-form name="myform">
          ...
          <div>
            ...
            <v-text-field
              ...
              v-model.number="iasWeaponPrimary"
              name="wIAS1"
              type="number"
          ...
          <div v-if="canDualWield === true">
            <v-row>
              <v-col
                class="py-0 pl-3 pb-2"
                cols="10"
              >
                <h3>Secondary Weapon</h3>
              </v-col>
              <v-col
                v-if="weaponsSecondarySelected > 0"
                class="py-0"
                cols="2"
                align="right"
              >
      ...
      <v-col class="px-8">
        ...
        <p>
          ...
          <template v-if="attackSkill.note">
            Note: <span id="currentNote">{{ attackSkill.note }}</span>
          </template>
        </p>
        <h3>Table</h3>
        <breakpoints-table
          ...
          :attack-skill="data.attack[skillsSelected]"
        />
      </v-col>
    </v-row>
  </v-container>
...
export default {
  ...
  props: {
    w1: { default: '0', type: String },
    w2: { default: '0', type: String },
    c: { default: '0', type: String },
    ss: { default: '0', type: String },
    s: { default: '0', type: String },
    io: { default: '0', type: String },
    i1: { default: '0', type: String },
    i2: { default: '0', type: String },
    bh: { default: '0', type: String },
    bug: { default: '0', type: String },
    fana: { default: '0', type: String },
    frenz: { default: '0', type: String }, // frenzy was taken so we can only be frenz
    wolf: { default: '0', type: String },
    bos: { default: '0', type: String },
    freeze: { default: '0', type: String },
    decrep: { default: '0', type: String },
    debug: { default: '0', type: String }
  },
  data: () => ({
    isDebug: false,
  ...
  computed: {
    ...
    iasSecondaryAndOffWeapon: function () {
      if (this.weaponsSecondarySelected > 0) return this.iasOffWeapon + this.iasWeaponPrimary
      return 0
    },
    iasEffectivePrimary: function () {
      return Math.floor(120 * this.iasPrimaryAndOffWeapon / (120 + this.iasPrimaryAndOffWeapon))
    },
    iasEffectiveSecondary: function () {
      return Math.floor(120 * this.iasSecondaryAndOffWeapon / (120 + this.iasSecondaryAndOffWeapon))
    },
    query: function () {
      const query = {}
    ...
    fanaticism: function () {
      const values = []
      for (let i = 0; i <= 50; i++) {
        let v = Math.floor(Math.floor((110 * i) / (6 + i)) * (40 - 10) / 100) + 10
        if (i === 0) {
          v = 0
        }
    ...
    burstOfSpeed: function () {
      if (this.charactersSelected === 1) { // sin
        const values = []
        for (let i = 0; i <= 50; i++) {
          let v = Math.floor(Math.floor((110 * i) / (6 + i)) * (60 - 15) / 100) + 15
          if (i === 0) {
            v = 0
          }
          values.push({ value: v, text: i })
    ...
    weaponInfoPrimary: function () {
      const weapon = this.weaponsPrimary.find(w => w.value === this.weaponsPrimarySelected)
      if (weapon != null) {
        return {
    ...
    weaponInfoSecondary: function () {
      if (this.weaponsSecondarySelected != null) {
        const weapon = this.weaponsSecondary.find(w => w.value === this.weaponsSecondarySelected)
        if (weapon != null) {
          return {
        ...
        case 1: // Assassin
          ...
          if (this.shapeShiftFormsSelected === 0) {
            ...
            if ((weapPrimary.type === this.weaponTypes.claw) && (weapSecondary.type === this.weaponTypes.claw)) {
              values.push(this.getSkillOptionData('Dual Strike'))
            }
    ...
    startingFrame: function () {
      ...
      let start = 0
      const attackSkill = this.data.attack[this.skillsSelected]
      const weapPrimary = this.lookupWeapon[this.weaponsPrimarySelected]
      ...
      // Old BoI, Impale, Jab, old Fists of Ember, old Fists of Thunder, Dual Strike, Double Swing, Frenzy, Double Throw, Whirlwind (potential 2 hand attacks?)
    ...
    breakpoints: function () {
      ...
      let temp
      let temp1
      let OIAS = this.iasOffWeapon
      const WIAS = this.iasWeaponPrimary
      const attackSkill = this.data.attack[this.skillsSelected]
      ...
      const nonStandardWeapon = []
      const weapPrimary = this.lookupWeapon[this.weaponsPrimarySelected]
      const start = this.startingFrame
      ...
      try {
        ...
        if (this.shapeShiftFormsSelected === 0) {
          ...
          if (attackSkill.title === 'Frenzy (first swing hits)') {
            // this doesn't seem to get hit
            console.log('special case breakpoints for frenzy first swing hits returning early')
            return {}
          }
          ...
          if (attackSkill.animation === 1 &&
            ...
            for (let i = Math.min(Math.max(100 + this.SIAS - this.WSMprimaer, 15), 175); i <= 175; i++) {
              ...
              resultFpa = (resultFpa + temp) / 2
              if ((temp1 !== resultFpa) && (i - 100 - this.SIAS + this.WSMprimaer < 120)) {
                breakpoints1[breakpoints1.length] = [Math.ceil(120 * (i - 100 - this.SIAS + this.WSMprimaer) / (120 - (i - 100 - this.SIAS + this.WSMprimaer))) - this.iasPrimaryAndOffWeapon, resultFpa]
                temp1 = resultFpa
              }
            ...
            temp1 = 0
            for (let i = Math.min(Math.max(100 + this.SIAS - this.WSMsekundaer, 15), 175); i <= 175; i++) {
              resultFpa = this.calcFPA(12, i, 0)
              if ((temp1 !== resultFpa) && (i - 100 - this.SIAS + this.WSMsekundaer < 120)) {
                breakpoints2[breakpoints2.length] = [Math.ceil(120 * (i - 100 - this.SIAS + this.WSMsekundaer) / (120 - (i - 100 - this.SIAS + this.WSMsekundaer))) - this.iasSecondaryAndOffWeapon, resultFpa]
                temp1 = resultFpa
              }
          ...
          if (attackSkill.rollback === 0) {
            ...
            for (let i = Math.min(Math.max(100 + this.SIAS - this.WSMprimaer, 15), 175); i <= 175; i++) {
              ...
              if ((temp1 !== resultFpa) && (i - 100 - this.SIAS + this.WSMprimaer < 120)) {
                ...
                breakpointsAPS[breakpointsAPS.length] = parseInt(2500 / ((this.rollback1 + this.rollback2 * 3 + this.rollback3) / 5), 10) / 100
                temp1 = resultFpa
              }
            }
          }
          if (attackSkill.title === 'Strafe') {
            ...
            for (let i = Math.min(Math.max(100 + this.SIAS - this.WSMprimaer, 15), 149); i <= 149; i++) {
              this.animationFrames = this.aktionsframe[weapPrimary.type][this.charactersSelected]
              this.rollback1 = this.calcFPA(this.animationFrames, i, start)
              this.rollback1++
              ...
              if (temp1 !== resultFpa) {
                ...
                breakpointsAPS[breakpointsAPS.length] = parseInt(2500 / ((this.rollback1 + this.rollback2 + this.rollback3 * 4 + this.rollback4 * 3 + this.rollback5) / 10), 10) / 100
                temp1 = resultFpa
              }
            }
          }
          if (attackSkill.title === 'Fend') {
            ...
            for (let i = Math.min(Math.max(100 + this.SIAS - this.WSMprimaer, 15), 175); i <= 175; i++) {
              ...
              if (temp1 !== resultFpa) {
                ...
                breakpointsAPS[breakpointsAPS.length] = parseInt(2500 / ((this.rollback1 + this.rollback2 + this.rollback3) / 3), 10) / 100
                temp1 = resultFpa
              }
            }
          }
        ...
        // wereform table
        if (this.shapeShiftFormsSelected > 0) {
          while (parseInt(OIAS / 5, 10) !== parseFloat(OIAS / 5)) {
            OIAS--
          }
          ...
          if ((this.weaponsSecondarySelected > 0) && (attackSkill.animation === 1)) {
            ...
            let AnimSpeed = 256
            if (weapPrimary.type === 1) { // assassin claws?
              AnimSpeed = 208
            }
            const iasRows = 50 // x + 1 rows shown. increased from 24 to show higher ias values
            let tempframe
            let tempframe2
            for (let i = 0; i <= iasRows; i++) {
              for (let j = 0; j <= 14; j++) {
                if (attackSkill.title === 'Feral Rage') {
                  ...
                  if ((OIAS > 70) && (j === 0)) {
                    breakpoints2[breakpoints2.length] = Math.ceil(256 * 7 / Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + 5 * i - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (5 * i + parseInt(OIAS, 10)) / (120 + (5 * i + parseInt(OIAS, 10)))), 15), 175) / 100)) + Math.ceil((256 * 13 - Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + 5 * i - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (5 * i + parseInt(OIAS, 10)) / (120 + (5 * i + parseInt(OIAS, 10)))), 15), 175) / 100) * Math.ceil(256 * 7 / Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + 5 * i - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (5 * i + parseInt(OIAS, 10)) / (120 + (5 * i + parseInt(OIAS, 10)))), 15), 175) / 100))) / (2 * Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + 5 * i - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (5 * i + parseInt(OIAS, 10)) / (120 + (5 * i + parseInt(OIAS, 10)))), 15), 175) / 100))) - 1
                  }
                }
                if (attackSkill.title === 'Fury') {
                  ...
                  if ((OIAS > 70) && (j === 0)) {
                    temp = (Math.ceil(256 * 7 / Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + 5 * i - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (5 * i + parseInt(OIAS, 10)) / (120 + (5 * i + parseInt(OIAS, 10)))), 15), 175) / 100)) * 4 + Math.ceil(256 * 13 / Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + 5 * i - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (5 * i + parseInt(OIAS, 10)) / (120 + (5 * i + parseInt(OIAS, 10)))), 15), 175) / 100)) - 1) / 5
                    if (parseInt(temp, 10) === parseFloat(temp)) {
                      temp = temp + '.0'
                    }
                ...
                if ((attackSkill.title !== 'Feral Rage') && (attackSkill.title !== 'Fury')) {
                  ...
                  if ((OIAS > 70) && (j === 0)) {
                    breakpoints2[breakpoints2.length] = Math.ceil(256 * tempframe / Math.floor(Math.floor(256 * tempframe2 / Math.floor(256 * this.animationFrames / Math.floor((100 + 5 * i - this.WSMprimaer) * AnimSpeed / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (5 * i + parseInt(OIAS, 10)) / (120 + (5 * i + parseInt(OIAS, 10)))), 15), 175) / 100)) - 1
                  }
                }
              }
            }
            ...
            for (let k = 0; k <= 14; k++) {
              if ((parseInt(WIAS / 5, 10) !== parseFloat(WIAS / 5)) && (this.skillsSelected === 26)) { // feral rage
                nonStandardWeapon.push(Math.ceil(256 * 7 / Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + parseInt(WIAS, 10) - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (parseInt(WIAS, 10) + 5 * k) / (120 + (parseInt(WIAS, 10) + 5 * k))), 15), 175) / 100)) + Math.ceil((256 * 13 - Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + parseInt(WIAS, 10) - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (parseInt(WIAS, 10) + 5 * k) / (120 + (parseInt(WIAS, 10) + 5 * k))), 15), 175) / 100) * Math.ceil(256 * 7 / Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + parseInt(WIAS, 10) - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (parseInt(WIAS, 10) + 5 * k) / (120 + (parseInt(WIAS, 10) + 5 * k))), 15), 175) / 100))) / (2 * Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + parseInt(WIAS, 10) - this.WSMprimaer) * 256 / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (parseInt(WIAS, 10) + 5 * k) / (120 + (parseInt(WIAS, 10) + 5 * k))), 15), 175) / 100))) - 1)
              }
              if ((parseInt(WIAS / 5, 10) !== parseFloat(WIAS / 5)) && (this.skillsSelected === 29)) { // fury
                temp = (Math.ceil(256 * 7 / Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + parseInt(WIAS, 10) - this.WSMprimaer) * 256 / 100))) * Math.min(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (parseInt(WIAS, 10) + 5 * k) / (120 + (parseInt(WIAS, 10) + 5 * k))), 175) / 100)) * 4 + Math.ceil(256 * 13 / Math.floor(Math.floor(256 * 9 / Math.floor(256 * this.animationFrames / Math.floor((100 + parseInt(WIAS, 10) - this.WSMprimaer) * 256 / 100))) * Math.min(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (parseInt(WIAS, 10) + 5 * k) / (120 + (parseInt(WIAS, 10) + 5 * k))), 175) / 100)) - 1) / 5
                if (parseInt(temp, 10) === parseFloat(temp)) {
                  temp = temp + '.0'
                }
                nonStandardWeapon.push(temp)
              }
              if ((parseInt(WIAS / 5, 10) !== parseFloat(WIAS / 5)) && (this.skillsSelected !== 26) && (this.skillsSelected !== 29)) {
                console.warn('nonStandardWeapon ias')
                nonStandardWeapon.push(Math.ceil(256 * tempframe / Math.floor(Math.floor(256 * tempframe2 / Math.floor(256 * this.animationFrames / Math.floor((100 + parseInt(WIAS, 10) - this.WSMprimaer) * AnimSpeed / 100))) * Math.min(Math.max(100 - this.WSMprimaer + this.SIAS + Math.floor(120 * (parseInt(WIAS, 10) + 5 * k) / (120 + (parseInt(WIAS, 10) + 5 * k))), 15), 175) / 100)) - 1)
              }
            }
          }
        }
        ...
        return {
          ...
          breakpointsAPS,
          nonStandardWeapon, // ias not a multiple of 5
          oIas: OIAS
        }
      } catch (error) {
        console.error(error)
    ...
    standardisedBreakpoints: function () {
      ...
      if (this.attackSkill.title === 'Frenzy (first swing hits)') {
        ...
        const maxSecondaryIasToCalculate = 70
        const rowIasIncrement = 5
        const colIasIncrement = 5
        const numberOfRowsToCalculate = maxPrimaryIasToCalculate / rowIasIncrement
        ...
        let isMaxRows = false
        for (let i = 0; i <= numberOfRowsToCalculate; i++) { // iterate down the table
          const ias1 = i * rowIasIncrement
          const frames = []
          const aps = 0
          for (let ias2 = 0; ias2 <= Math.max(maxSecondaryIasToCalculate, this.iasWeaponSecondary); ias2++) { // iterate across the table
            if ((ias2 % colIasIncrement === 0 && ias2 <= maxSecondaryIasToCalculate) || ias2 === this.iasWeaponSecondary) {
              const bp = this.frenzyFpa(this.iasOffWeapon, ias1, ias2)
              frames.push(`${bp.parts[0]}/${bp.parts[1]}`)
              // add column headers on first row pass
              if (ias1 === 0) {
                bps.meta.columnHeaders.push(ias2)
              }
              // check if max fpa reached
              if (bp.isPrimaryFpaMaxed) {
                isMaxRows = true
              }
            }
          }
        ...
        if (this.iasWeaponPrimary % rowIasIncrement !== 0 || this.iasWeaponPrimary > maxPrimaryIasToCalculate) {
          const ias1 = this.iasWeaponPrimary
          const frames = []
          const aps = 0
          for (let ias2 = 0; ias2 <= Math.max(maxSecondaryIasToCalculate, this.iasWeaponSecondary); ias2++) { // iterate across the table
            if ((ias2 % colIasIncrement === 0 && ias2 <= maxSecondaryIasToCalculate) || ias2 === this.iasWeaponSecondary) {
        ...
        for (let j = 1; j < iasRows + 2; j++) {
          const ias = 5 * (j - 1)
          const bpRow = this.breakpoints.breakpoints.slice((j - 1) * 15, ((j - 1) * 15) + 15)
          const frames = []
          const aps = 0
        ...
        if (this.iasWeaponPrimary % 5 !== 0) {
          const ias = this.iasWeaponPrimary
          const frames = this.breakpoints.nonStandardWeapon
          const aps = 0
          if (this.iasOffWeapon > 70) {
        ...
        const currentBp = bps.rows.find(bp => bp.ias === this.iasWeaponPrimary)
        if (currentBp) {
        ...
        for (let i = 0; i < this.breakpoints.breakpoints.length; i++) {
          const ias = this.breakpoints.breakpoints[i][0]
          const frames = [this.breakpoints.breakpoints[i][1]]
          const aps = parseInt(2500 / (this.aidel + this.breakpoints.breakpoints[i][1]), 10) / 100
          bps.rows.push({
            ias,
            frames,
            aps,
        ...
        for (let i = 0; i < this.breakpoints.breakpoints.length; i++) {
          const ias = this.breakpoints.breakpoints[i][0]
          const frames = [this.breakpoints.breakpoints[i][1]]
          const aps = this.breakpoints.breakpointsAPS[i]
          bps.rows.push({
            ias,
            frames,
            aps,
  ...
  methods: {
    ...
    swapWeapons: function () {
      const tempWeaponsPrimarySelected = this.weaponsPrimarySelected
      const tempIasWeaponPrimaryRaw = this.iasWeaponPrimaryRaw
    ...
    getSkillOptionData: function (title) {
      const skill = this.data.attack.find(a => a.title === title)
    ...
    calculateWsm: function () {
      ...
      if (((this.charactersSelected === 1) || (this.charactersSelected === 2)) && (this.weaponsSecondarySelected > 0)) {
        if (this.isWsmBug) {
          ...
          this.WSMprimaer = parseInt((weapPrimary.wsm + weapSecondary.wsm) / 2, 10) + weapPrimary.wsm - weapSecondary.wsm
          this.WSMsekundaer = parseInt((weapPrimary.wsm + weapSecondary.wsm) / 2, 10)
        } else {
          this.WSMprimaer = parseInt((weapPrimary.wsm + weapSecondary.wsm) / 2, 10)
          this.WSMsekundaer = parseInt((weapPrimary.wsm + weapSecondary.wsm) / 2, 10) + weapSecondary.wsm - weapPrimary.wsm
        }
      }
    },
    animationSpeed: function (FramesPerDirection) {
      ...
      if (this.shapeShiftFormsSelected === 1) {
        ...
        animationSpeed = Math.floor(256 * 10 / Math.floor(256 * FramesPerDirection / Math.floor((100 + this.iasPrimaryAndOffWeapon - parseInt(this.iasOffWeapon, 10) - weapPrimary.wsm) * animationSpeed / 100)))
      }
      // Wolf
      if (this.shapeShiftFormsSelected === 2) {
        if (weapPrimary.type === this.weaponTypes.twoHandedSword) { // 2-hand swords
        ...
        animationSpeed = Math.floor(256 * 9 / Math.floor(256 * FramesPerDirection / Math.floor((100 + this.iasPrimaryAndOffWeapon - parseInt(this.iasOffWeapon, 10) - weapPrimary.wsm) * animationSpeed / 100)))
      }
    ...
    calcFPA: function (FramesPerDirection, Acceleration, StartingFrame) {
      ...
      if (this.shapeShiftFormsSelected === 1) {
        ...
        // animationSpeed = Math.floor(256 * 10 / Math.floor(256 * FramesPerDirection / Math.floor((100 + this.iasPrimaryAndOffWeapon - parseInt(this.iasOffWeapon) - weapPrimary.wsm) * animationSpeed / 100)));
      ...
      if (this.shapeShiftFormsSelected === 2) {
        ...
        // animationSpeed = Math.floor(256 * 9 / Math.floor(256 * FramesPerDirection / Math.floor((100 + this.iasPrimaryAndOffWeapon - parseInt(this.iasOffWeapon) - weapPrimary.wsm) * animationSpeed / 100)));
      ...
      let FPA = Math.ceil(256 * (FramesPerDirection - StartingFrame) / Math.floor(animationSpeed * Acceleration / 100)) - 1
      let FPAmax = Math.ceil(256 * (FramesPerDirection - StartingFrame) / Math.floor(animationSpeed * 175 / 100)) - 1
      if (attackSkill.title === 'Frenzy (first swing hits)') {
    ...
    frenzyFpa: function (OIAS, iasWeaponPrimary = null, iasWeaponSecondary = null) {
      const weapPrimary = this.lookupWeapon[this.weaponsPrimarySelected]
      const weapSecondary = this.lookupWeapon[this.weaponsSecondarySelected]
      const wsmPrimary = weapPrimary.wsm
      const wsmSecondary = weapSecondary.wsm
      const IASprimary = iasWeaponPrimary || 0 // only required to find the current fpa
      const IASsecondary = iasWeaponSecondary || 0
      this.calculateSkillIas()
      const SIAS = this.SIAS
      let WSM1 = 0
      let WSM2 = 0
      if (!this.isWsmBug) {
        // If the primary weapon is equipped in the left weapon slot:
        WSM1 = wsmPrimary + wsmSecondary - (wsmPrimary + wsmSecondary) / 2
        WSM2 = 2 * wsmSecondary - (wsmPrimary + wsmSecondary) / 2
      } else {
        // If the primary weapon is equipped in the right weapon slot:
        WSM1 = (wsmPrimary + wsmSecondary) / 2 + wsmPrimary - wsmSecondary
        WSM2 = (wsmPrimary + wsmSecondary) / 2
      }
      const EIAS1 = Math.floor(120 * (OIAS + IASprimary) / (120 + OIAS + IASprimary))
      const EIAS2 = Math.floor(120 * (OIAS + IASsecondary) / (120 + OIAS + IASsecondary))
      const acceleration1 = Math.max(Math.min(100 + SIAS + EIAS1 - WSM1, 175), 15) // 100 is normally 70 here because of the sequence, -30 penalty is applied to the skill ias
      const acceleration2 = Math.max(Math.min(100 + SIAS + EIAS2 - WSM2, 175), 15) // 100 is normally 70 here because of the sequence, -30 penalty is applied to the skill ias
      const fpa1 = Math.ceil(256 * 9 / Math.floor(256 * acceleration1 / 100)) - 1
      const fpa2 = Math.ceil((256 * 17 - fpa1 * Math.floor(256 * acceleration1 / 100)) / Math.floor(256 * acceleration2 / 100))
      const resultFpa = fpa1 + fpa2
      return {
        sum: resultFpa,
        parts: [
          fpa1,
        ...
        isPrimaryFpaMaxed: acceleration1 === 175,
    ...
    calculateValues: function () {
      ...
      this.calculateSkillIas()
      this.calculateWsm()
      let acceleration = Math.max(Math.min(100 + this.SIAS + this.iasEffectivePrimary - this.WSMprimaer, 175), 15)
      const acceleration2 = Math.max(Math.min(100 + this.SIAS + this.iasEffectiveSecondary - this.WSMsekundaer, 175), 15)
      const start = this.startingFrame
      let rollbackframe
      ...
      if (attackSkill.title === 'Frenzy (first swing hits)') {
        ...
        const frenzyFpa = this.frenzyFpa(this.iasOffWeapon, this.iasWeaponPrimary, this.iasWeaponSecondary)
        this.rollback1 = frenzyFpa.parts[0]
        this.rollback3 = frenzyFpa.parts[1]
        this.isCurrentFpaMaxed = frenzyFpa.isPrimaryFpaMaxed && frenzyFpa.isSecondaryFpaMaxed
        ...
        // 9 Fists of Ember, 10 Fists of Thunder, 11 Blades of Ice, 12 Dual Strike && offhand weapon selected
        if ((this.skillsSelected > 8) && (this.skillsSelected < 13) && (this.weaponsSecondarySelected > 0)) {
          this.animationFrames = 16
        }
        // merc act 2
      ...
      if (attackSkill.rollback === 0) {
        ...
        if (this.skillsSelected === 14) {
          ...
          this.currentAps = parseInt(100 * 25 / ((this.rollback1 * 4 + this.rollback3) / 5), 10) / 100 + ' attacks per second'
        }
        // Fury
        ...
        if (this.skillsSelected === 24) {
          ...
          let isBarbTwoHandedSwordAsOneHanded = false
          if ((weapPrimary.type === this.weaponTypes.twoHandedSword) && (this.weaponsPrimaryBarbHandednessSelected === 1)) {
          ...
          // Zeal
          if (this.skillsSelected === 24) {
            this.currentAps = parseInt(100 * 25 / ((this.rollback1 + this.rollback2 * 3 + this.rollback3) / 5), 10) / 100 + ' attacks per second'
          }
        }
      }
      // Strafe
      if (attackSkill.rollback === 50) {
        ...
        this.currentAps = parseInt(100 * 25 / ((this.rollback1 + this.rollback2 + this.rollback3 * 4 + this.rollback4 * 3 + this.rollback5) / 10), 10) / 100 + ' attacks per second'
      }
      // Fend
      if (attackSkill.rollback === 40) {
        ...
        this.currentAps = parseInt(100 * 25 / ((this.rollback1 + this.rollback2 + this.rollback3) / 2), 10) / 100 + ' attacks per second'
      }
      // Most attacks
    ...
    calculateSkillIas: function () {
      const fana = parseInt(this.fanaticismSkillIas, 10)
      const frenzy = parseInt(this.frenzySkillIas, 10)
      let wolf = parseInt(this.werewolfSkillIas, 10)
      const tempo = parseInt(this.burstOfSpeedSkillIas, 10)
      const holyfrost = parseInt(this.holyFreezeSkillIas, 10)
      const attackSkill = this.data.attack[this.skillsSelected]
      // != Wolf
      ...
      // impale, jab, Dual Strike, double swing, double throw, frenzy, whirlwind
    ...
    sanitiseSelected: function (selected, values) {
      if (typeof (values.find(v => v.value === selected)) === 'undefined') {
        const valuesMap = values.filter(v => v.value !== undefined) // remove headers without values
